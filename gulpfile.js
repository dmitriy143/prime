...
const path = {
  ...
  src: {
    ...
    js: [source_folder + "/js/*.js", source_folder + "/blocks/**/*.js", "!" + source_folder + "js/libs"],
...
function libs() { //подключаем библиотеки
  return src(['node_modules/jquery/dist/jquery.min.js'])
    .pipe(dest('_src/js/libs'))
}
...
function fontsStyle(done) {
  fs.readdir(path.build.fonts, (err, items) => {
    ...
    if (file_content == '') {
      ...
      return [console.log('file \'fonts.scss\' is not empty'), done()]
    }
  })
}
...
const build = series(clean, parallel(pug, libs, js, css, images, fonts), webpHtml, fontsStyle);
